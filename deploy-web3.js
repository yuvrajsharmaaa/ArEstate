...
async function main() {
    try {
        ...
        deployedContracts.identityRegistry = await deployContract('IdentityRegistry', [], deployer);
        
        // 3. Deploy Compliance (no dependencies)
        deployedContracts.compliance = await deployContract('Compliance', [], deployer);
        
        // 4. Deploy PropertyToken (requires IdentityRegistry and Compliance)
        deployedContracts.propertyToken = await deployContract('PropertyToken', [
        ...
        deployedContracts.leaseAgreement = await deployContract('LeaseAgreement', [], deployer);
        
        // 6. Deploy EscrowPayment (requires MockUSDC)
        deployedContracts.escrowPayment = await deployContract('EscrowPayment', [
