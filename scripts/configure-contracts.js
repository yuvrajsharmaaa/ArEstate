...
async function main() {
    ...
    try {
        ...
        const tx1 = await identityRegistry.grantRole(REGISTRAR_ROLE, deployer.address, { gasPrice });
        await tx1.wait();
        console.log("✅ REGISTRAR_ROLE granted");

        const tx2 = await identityRegistry.grantRole(COMPLIANCE_ROLE, deployer.address, { gasPrice });
        await tx2.wait();
        console.log("✅ COMPLIANCE_ROLE granted");

        const tx3 = await propertyToken.grantRole(MINTER_ROLE, deployer.address, { gasPrice });
        await tx3.wait();
        console.log("✅ MINTER_ROLE granted");

        const tx4 = await leaseAgreement.grantRole(LEASE_MANAGER_ROLE, deployer.address, { gasPrice });
        await tx4.wait();
        ...
        console.log("This is okay - the main contracts are deployed and functional!");
    }
